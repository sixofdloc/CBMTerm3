...
namespace FileTransferProtocols
{
    class CRC16
    {
        ...
        public static ushort CRC16_ccitt(byte[] buf, int index, int len)
        {
            ushort crc = 0;
            for (int counter = index; counter < len; counter++)
            {
                crc = (ushort)((crc << 8) ^ crc16tab[((crc >> 8) ^ (0xff & buf[index + counter]))]);
            }
            return crc;
        }


        public static ushort SimpleCRC(byte[] buf, int index, int len)
        {
